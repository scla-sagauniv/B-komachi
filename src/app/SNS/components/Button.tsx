...
const Modal = ({onButtonClick, canCloseByClickingBackground = true}:ModalProps) => {
  ...
  const elmModal = (
    <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full object-contain">
      <div className="flex justify-center items-center relative border-8 border-rose-500 z-30 w-2/3 h-2/5 bg-white rounded-md will-change-transform will-change-opacity animate-appear">
          <button
          ...
          <form className="flex justify-center items-center h-full w-full" onSubmit={(e) => closeAll(e)}>
            ...
            <button 
            type="submit"
            className="absolute bottom-1 right-2 text-[20px] bg-rose-500 rounded px-5 py-2 hover:bg-rose-400 text-white">post</button>
          </form>  
      </div>
